...
const AdicionarPet = () => {
  ...
  // Função auxiliar para obter emoji baseado no tipo - MOVIDA PARA O ESCOPO CORRETO
  ...
  const mudarDado = (e) => {
    ...
    // Resetar a raça se o tipo de pet mudar
    if (name === 'tipo') {
      setDadosPet(prev => ({
        ...prev,
        raca: ''
  ...
  const enviarForm = (e) => {
    ...
    // CRUCIAL: Uso correto da função getEmojiPorTipo (sem 'this')
    const novoPet = {
      nome: dadosPet.nome,
      tipo: dadosPet.tipo,
      raca: dadosPet.raca,
      idade: String(dadosPet.idade) + ' anos', // Formata a idade para seguir o padrão inicial
      descricao: dadosPet.bio,
      foto: imagemPreview || getEmojiPorTipo(dadosPet.tipo), // CORRIGIDO: Chamada da função
      // Campos extras
      ...
      dataNascimento: dadosPet.dataNascimento // Mantém data de nascimento
    };
    ...
    if (sucesso) {
      alert(`${dadosPet.nome} cadastrado(a) com sucesso!`);
      navigate('/perfil');
    } else {
      alert('Falha ao cadastrar o pet. Tente novamente.');
    }
  };
