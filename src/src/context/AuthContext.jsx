...
// Usuários pré-cadastrados - VERSÃO CORRIGIDA COM CAMPOS DE FOTO
const usuariosPreCadastrados = {
  "wenderviana@gmail.com": {
    ...
    fotoPerfil: null, // <<< CAMPO ADICIONADO
    fotoCapa: null,   // <<< CAMPO ADICIONADO
    pets: [
      {
        id: 1,
        nome: "Luna",
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (usuariosSalvos) {
      try {
        // CORREÇÃO: Mesclar dados salvos com dados pré-cadastrados,
        // garantindo que novos campos (como fotos) sejam mantidos.
        const salvos = JSON.parse(usuariosSalvos);
        usuariosCarregados = Object.keys(usuariosPreCadastrados).reduce((acc, email) => {
          acc[email] = {
            ...usuariosPreCadastrados[email],
            ...(salvos[email] || {}) // Sobrescreve com dados salvos (incluindo pets e fotos)
          };
          return acc;
        }, salvos); // Inclui usuários recém-cadastrados que só existem em `salvos`
  ...
  const salvarDados = (novosUsuarios, usuarioAtualizado = null) => {
    try {
      ...
      if (usuarioAtualizado) {
        // Garantir que a senha NÃO seja salva no localStorage do usuário logado por segurança
        const { senha, ...usuarioSemSenha } = usuarioAtualizado; 
        setUsuario(usuarioSemSenha);
        localStorage.setItem('usuarioPetRefugio', JSON.stringify(usuarioSemSenha));
      }
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if (usuarioEncontrado && usuarioEncontrado.senha === senha) {
          const { senha: _, ...usuarioLogado } = { // Remove senha
            email,
            ...usuarioEncontrado
          };
  ...
  const logout = () => {
    if (usuario) {
      ...
      // Usamos salvarDados para persistir o status offline no estado global de usuários
      try {
        // Precisamos reconstruir o objeto do usuário com a senha para salvar no estado global
        const usuarioCompleto = { ...usuarios[usuario.email], online: false };
        salvarDados(novosUsuarios, usuarioCompleto); // A senha será removida do localStorage de usuario
      } catch (e) {
  ...
  const adicionarFoto = async (tipo, base64) => {
    ...
    try {
      ...
      // Ajustar o limite para uma string base64 que representa um tamanho razoável
      if (base64.length > 500000) { // ~500KB
        ...
        // Reduzir resolução para algo mais razoável para um avatar/capa
        const maxDim = 800;
        let width = img.width;
  ...
  // ... (o restante das funções do Contexto é mantido)
